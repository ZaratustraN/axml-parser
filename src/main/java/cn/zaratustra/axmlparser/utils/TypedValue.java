 ...
 */
package cn.zaratustra.axmlparser.utils;

import cn.zaratustra.axmlparser.model.StartTagChunk;
...
public class TypedValue {
    ...
    public static void initAttribute(StartTagChunk.Attribute attribute, String valueString) throws Exception {
        if (valueString.startsWith("@")) {
            attribute.setType(TYPE_REFERENCE);
            attribute.setData(Integer.valueOf(valueString.substring(1)));
            attribute.setValueString(-1);
        } else if (valueString.startsWith("?")) {
            attribute.setType(TYPE_ATTRIBUTE);
            attribute.setData(Integer.valueOf(valueString.substring(1)));
            attribute.setValueString(-1);
        } else if (isDimension(valueString)) {
            throw new Exception("Do not support dimension value in AndroidManifest");
        } else if (isRadix(valueString)) {
            throw new Exception("Do not support radix value in AndroidManifest");
        } else if (valueString.startsWith("#")) {
            throw new Exception("Do not support color value in AndroidManifest");
        } else if (valueString.startsWith("0x")) {
            attribute.setType(TYPE_INT_HEX);
            attribute.setData(Integer.decode(valueString));
            attribute.setValueString(-1);
        } else if (valueString.equals("true") || valueString.equals("false")) {
            attribute.setType(TYPE_INT_BOOLEAN);
            attribute.setData(valueString.equals("true") ? -1 : 0);
            attribute.setValueString(-1);
        } else if (isInteger(valueString)) {
            attribute.setType(16);
            attribute.setData(Integer.valueOf(valueString));
            attribute.setValueString(-1);
        } else if (isFloat(valueString)) {
            attribute.setType(TYPE_FLOAT);
            attribute.setData(Float.floatToIntBits(Float.valueOf(valueString)));
            attribute.setValueString(-1);
        } else {
            attribute.setType(TYPE_NULL);
            throw new Exception("nothing get in attribute");
        }
    }

    private static boolean isRadix(String valueString) {
        for (int i = 0; i < FRACTION_UNIT_STRS.length; i++) {
            if (valueString.endsWith(FRACTION_UNIT_STRS[i])) {
                return true;
            }
        }
        return false;
